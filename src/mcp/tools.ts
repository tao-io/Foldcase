...
const ListShowcases = Tool.make("foldcase_list_showcases", {
  description:
    "List every Showcase in the catalog, with the directory they were read from, each flagged with whether it carries an introspectable Message schema and a Model schema, and with the Message tags its play declares it dispatches. `gaps` answers the question the catalog exists to answer: per component, `undispatched` names the Messages of its union that no play dispatches — write a Showcase for each, that is the work — and `unknown` names declared tags the union does not carry, which is a typo or a Message renamed since, so fix the declaration. A component where no Showcase declares its dispatches is absent from `gaps` entirely: nobody said is not the same as fully showcased. The entry point for driving Foldcase: enumerate, then foldcase_get_showcase_schema or foldcase_get_showcase_model_schema to learn a payload shape, then foldcase_run_showcase to assert one play, or foldcase_run_catalog to assert them all. It answers from the catalog this server has loaded: foldcase_load_catalog picks up a file that appeared or vanished, but a Showcase renamed inside a file that was already loaded is listed under its old id until the server restarts. The run verbs are never stale — they read the files from disk.",
